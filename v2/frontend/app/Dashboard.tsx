...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setLoading(true);
      try {
        const response = await fetch(`${API}/api/v2/places?${params}`, { signal: controller.signal });
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/v2/stats`).then((response) => response.json()),
      fetch(`${API}/api/v2/facets`).then((response) => response.json()),
      ...
      <div className="content">
        <aside className="sidebar">
          <div className="filters">
            <input aria-label="施設を検索" placeholder="施設名・住所で検索" value={query} onChange={(event) => setQuery(event.target.value)} />
            <select aria-label="都道府県" value={prefecture} onChange={(event) => setPrefecture(event.target.value)}>
              <option value="">すべての都道府県</option>
              {facets.prefectures.map((item) => <option key={item.value} value={item.value}>{item.value} ({item.count})</option>)}
            </select>
          ...
          <div className="stats">
            {stats ? `全${stats.record_count}件・評価済み${stats.scored_count}件${stats.average_score == null ? "" : `・平均${stats.average_score.toFixed(1)}点`}` : "統計を読み込み中"}
          </div>
          <div className="cards">
            ...
            {!loading && places.map((place) => (
              <article className="card" key={place.id}>
                ...
                <p className="score">きれい度: {place.toilet_score == null ? "未評価" : `${place.toilet_score}点`}</p>
              </article>
            ))}
