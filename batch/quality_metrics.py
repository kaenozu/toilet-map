...
def collect_sqlite_metrics(db_path: str) -> dict | None:
    ...
    try:
        ...
        scored = connection.execute(
            "SELECT COUNT(*) FROM toilets WHERE confidence > 0"
        ).fetchone()[0]
        public_toilets = connection.execute(
            "SELECT COUNT(*) FROM toilets WHERE is_public_toilet = 1"
        ).fetchone()[0]
        ...
        return {
            ...
            "prefecture_counts": {
                str(prefecture or ""): int(count)
                for prefecture, count in prefecture_rows
            },
...
def compare_sqlite_metrics(
    meta: dict, sqlite_metrics: dict
) -> tuple[list[str], list[str]]:
    errors: list[str] = []
    warnings: list[str] = []
    for meta_key, sqlite_key in (
        ("total", "total"),
        ("scored", "scored"),
        ("public_toilets", "public_toilets"),
    ):
        ...
        if expected is not None and expected != actual:
            errors.append(
                f"SQLite {sqlite_key} mismatch: json={expected}, db={actual}"
            )
    ...
    if json_last_updated and sqlite_last_updated and json_last_updated != sqlite_last_updated:
        warnings.append(
            f"SQLite last_updated mismatch: json={json_last_updated}, db={sqlite_last_updated}"
        )
    ...
    for prefecture in sorted(set(json_counts) | set(sqlite_counts)):
        ...
        if expected is None:
            ...
            errors.append(
                f"SQLite missing prefecture: {prefecture} (json={expected})"
            )
        elif expected != actual:
            errors.append(
                f"SQLite prefecture count mismatch: {prefecture}: json={expected}, db={actual}"
            )
...
def evaluate_quality_gate(
    metrics: dict, expected_prefectures: Iterable[str]
) -> tuple[list[str], list[str]]:
    ...
    checks = [
        (
            "Missing score",
            _rate(metrics.get("missing_score", 0), total),
            MAX_MISSING_SCORE_RATE,
        ),
        (
            "Missing prefecture",
            _rate(metrics.get("missing_prefecture", 0), total),
            MAX_MISSING_PREFECTURE_RATE,
        ),
        (
            "Missing address",
            _rate(metrics.get("missing_address", 0), total),
            MAX_MISSING_ADDRESS_RATE,
        ),
        (
            "Duplicate",
            _rate(len(metrics.get("duplicates", [])), total),
            MAX_DUPLICATE_RATE,
        ),
    ]
